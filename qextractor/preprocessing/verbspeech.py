...
VSPEECH_FILE = resource_filename('qextractor', 'data/verbos_dizer_ACDC.txt')
LABEL_FILE = resource_filename('qextractor', 'data/Label-Delaf_pt_v4_1.dic')

def remove_accents(s):
    """Auxiliary function that removes acents.

    Args:
        s: A string object
    Returns:
        The s string without accents
    """
    return s.replace("á", "a").replace("í", "i").replace("é", "e").replace("ó", "o").replace("ú", "u")

def load(file=LABEL_FILE,fvspeech=VSPEECH_FILE):
