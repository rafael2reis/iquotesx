...
def sortTasksByEnd(tasks):
    """Sorts tasks by their end times.

    Args:
        tasks: A list of tasks
    Returns:
        The tasks sorted by end time.
    """
    return sorted(tasks, key=lambda task: task.end)

def createPreviousArray(tasks):
    """Creates the previous array for the WISP.
...
def previous(i, tasks):
    """Returns the index of the task that ends before the start of task i.

    Args:
        i: The index of the task
        tasks: A list of tasks
    Returns:
        The index of the task previous of task i.
    """
    if i == 1 or i == 0:
        return 0
    else:
